# Function Calling 与 MCP：Skills 本质差异与最佳实践

![图片](https://mmbiz.qpic.cn/mmbiz_jpg/Z6bicxIx5naJ5iaVvfJqicERScicd5gI675ySRTQHRzmQ6zcfZt7p6BgxkEibibg1DDpVrahjIAY65cR5VylOYbrQW1Q/640?wx_fmt=jpeg&from=appmsg&wxfrom=13&tp=wxpic#imgIndex=0)
...
// 2. 根据工具名称找到对应的函数
const tools = {  get_weather: (city, date) => {    // 执行天气查询逻辑   
...
Claude 判断是否需调用某 Skill（基于请求内容匹配已加载的 skill_name 与 description）
    ↓
    若需要，则通过 Function Calling 调用 load_skill(skill_name)↓将对应 SKILL.md 的内容注入当前上下文，作为执行指令依据
    ↓
    Claude 依照 SKILL.md 中定义的流程执行任务↓在执行过程中，按需通过 Function Calling：  
        • 调用 bash 执行附带脚本  
...
通过前面的分析，我们可以看到Function Calling、MCP和Skills三者之间的本质关系：
