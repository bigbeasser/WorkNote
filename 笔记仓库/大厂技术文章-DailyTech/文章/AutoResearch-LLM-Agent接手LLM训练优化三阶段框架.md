...
# AutoResearch-LLM：Agent接手LLM训练优化三阶段框架

> **原文链接**: https://mp.weixin.qq.com/s/9qEgOV9FGk6u9_9zMLSI6A
> **作者**: 吉梦林
> **发布日期**: 2026-07-06
> **原标题**: AutoResearch-LLM：让 Agent 接手 LLM 训练优化
> **一句话总结**: 把 LLM 微调流水线沉淀成由 Agent 驱动的三阶段框架（场景诊断 → 方案设计 → 自动化实验），用 SKILL.md 固化调参清单与踩坑经验，让 Agent 自动完成配置、提交、监控、抓指标等重复工作，研究员专注于判断性工作。
> **前置知识检查**:
> - [ ] SKILL.md / Skill 工程（Agent 的 SOP、参数清单、Debug 表写成可执行规范文件）
> - [ ] LLM 微调基础（SFT 全参 / SFT LoRA / DPO / 蒸馏 / RL（GRPO）的差异与适用场景）
> - [ ] 分布式训练（DeepSpeed ZeRO-2/3、Megatron Turbo、EP 及 `eff_batch` 公式）
> - [ ] Qwen3 thinking mode（默认开启，generate 时先吐 `<think>...</think>` 影响指标）
> - [ ] Agent /loop 能力（定时循环唤起，周期检查任务状态、收尾写指标）
...
![图1](<assets/AutoResearch-LLM-Agent接手LLM训练优化三阶段框架/img_01.png>)
...
![图2](<assets/AutoResearch-LLM-Agent接手LLM训练优化三阶段框架/img_02.png>)
...
| 训练框架 | 单文件main.py（PyTorch） | TuningFactory（submodule 引入）+ ROLL（阿里开源的 RL 训练框架） |
...
![图3](<assets/AutoResearch-LLM-Agent接手LLM训练优化三阶段框架/img_03.png>)
...
| 3 | 数据规模 | baseline 前 | 独立样本数对当前任务是否充分？ | 生成 注意：基座模型在本阶段就要锁定。后续整个实验序列里，`MODEL_NAME` 不再是一个可调变量。Agent 不允许擅自换基座；如果出现 "LoRA 全跑过仍饱和" 等信号，会专门起一节 "需要决策" 提示用户，由用户拍板。
...
```
# 状态 + application_id + job_namecurl -s "https:///nebula/meta?task_id=${TASK_ID}"# 实时 stdout / stderrcurl -s "https:///nebula/log?task_id=${TASK_ID}&rank=0&stream=stdout&bufferSize=102400"curl -s "https:///nebula/log?task_id=${TASK_ID}&rank=0&stream=stderr&bufferSize=102400"# 训练完成后下载完整日志 zipAPP_ID=$(curl -s "https:///nebula/meta?task_id=${TASK_ID}" | python3 -c "import sys,json;print(json.load(sys.stdin)['data']['application']['application_id'])")curl -L -o experiment_logs/${EXP}.zip "https:///logview/logs/download/${APP_ID}"
```
...
```
curl -s "https:///nebula/log?task_id=${TASK_ID}&rank=0&stream=stdout&bufferSize=204800" | python3 -c "import sys,json,red=json.load(sys.stdin)for log in d.get('logs',[ ]):    if log['role']=='worker':        c=log['result']['data']['content']        for m in re.finditer(r'predict_(\w+)\s+=\s+([\d.]+)', c):            print(f'  {m.group(1)}: {m.group(2)}')        break"
```
...
```
# === 模型配置 ===MODEL_NAME="Qwen/Qwen3-8B"PROMPT_TEMPLATE="qwen3"# === 任务配置 ===SCENARIO="query_rewrite"EXP_NAME="lora16_ds_2gpu"JOB_NAME="auto_research_${SCENARIO}_${EXP_NAME}"# === 数据源 ===ODPS_PROJECT=""INPUT="odps://${ODPS_PROJECT}/tables/auto_research_llm_${SCENARIO}/ds=20260624"EVAL_INPUT="odps://${ODPS_PROJECT}/tables/auto_research_llm_${SCENARIO}/ds=20260624"# === 训练超参 ===LR=2e-4
```
...
![图4](<assets/AutoResearch-LLM-Agent接手LLM训练优化三阶段框架/img_04.png>)
...
![图5](<assets/AutoResearch-LLM-Agent接手LLM训练优化三阶段框架/img_05.png>)
...
![图6](<assets/AutoResearch-LLM-Agent接手LLM训练优化三阶段框架/img_06.png>)

点击阅读原文即可体验！

> **图片说明**: 配图位于 assets/AutoResearch-LLM-Agent接手LLM训练优化三阶段框架/，CDN 可能过期
...
```mermaid
mindmap
  root((AutoResearch-LLM))
    三阶段流水线
      阶段1 场景理解与基线分析
        7维诊断 基座/数据质量/Prompt/训练方式/Eval/Baseline复盘
        跑baseline 拿eval_loss与任务指标
        锁定MODEL_NAME LOCKED参数
      ...
      阶段3 自动化实验验证
        单变量探索→正交组合→数据维度兜底
        生成config→push分支→submit→监控→评估→入csv
        产出 experiment_results.csv与final_report.md
    六级调参清单
      一级 LR/LoRA rank/target/epochs/Prompt/cutoff_len
      二级 batch相关
      三级 训练策略
      四级 数据维度
      五级 分布式
    ...
    训练方式矩阵
      ...
      SFT LoRA MoE
      DPO LoRA
      离线数据蒸馏 DashScope生成+SFT 已验证
      在线KD --stage distill 配置就绪
      RL GRPO 基于ROLL 接入未验证
    工程实现要点
      数据接入 ODPS三件套+OSS备选
      configs一文件一方式
      submodule分支自动sync
      星云HTTP API全链路监控
      Agent /loop循环验证
      Trainer predict_with_generate自动评估
      eff_batch经验公式
      平台skill兜底方法论
    踩坑实录
      PyTorch2.6+DeepSpeed weights_only反序列化
      Qwen3 thinking mode拉低BLEU
      多队列OSS endpoint差异
    横向对比
      karpathy/autoresearch 最简
      Pailitao-AutoResearch 低门槛
      前作CV demo CV分类
      auto_research_llm 本文 LLM接地气
      TAO-AutoResearch 最完整多Agent
```
...
**《[[大厂技术文章-DailyTech/文章/Loop Engineering-实操手册与14步落地清单|Loop Engineering-实操手册与14步落地清单]]》**：两者都用"循环"驱动 Agent 自主完成 ML/工程任务。本文用 Agent 自带的 `/loop 15m` 能力周期性检查训练 task_id 状态、自动收尾写 csv，正是 Loop Engineering 中"定时唤起 + 自动收尾"模式在 LLM 训练监控场景的具体落地；可对照阅读以理解 /loop 的更通用玩法。

**《[[大厂技术文章-DailyTech/文章/Skills-从编程工具配角到Agent研发核心|Skills-从编程工具配角到Agent研发核心]]》**：本文核心载体是 SKILL.md，把 Pipeline、6 级调参清单、Debug 表全部固化进一份 Skill 文件交给 Agent 执行，正是"Skill 作为 SOP"思想在 LLM 微调领域的实践。该文提供 Skill 工程化的理论框架，本文是其落地样本。

**《[[个人学习/LLM大模型类相关知识/开发人员微调大模型并暴露接口给后端/【DeepSeek+LoRA+FastAPI】开发人员如何微调大模型并暴露接口给后端调用|【DeepSeek+LoRA+FastAPI】开发人员如何微调大模型并暴露接口给后端调用]]》**：两者都涉及 LoRA 微调实操。本文在 1688 电商场景用 TuningFactory（LLaMA-Factory Fork）做 Qwen3 的 SFT LoRA / DPO LoRA，并给出 `lora_rank 8/16/32/64`、`lora_target=all-linear`、`LR 1e-4~5e-4` 等经验区间；该文是更轻量的单卡 DeepSeek+LoRA+FastAPI 路线，可互为补充理解 LoRA 微调从单机到分布式的演进。

**《[[大厂技术文章-DailyTech/文章/How LLMs Actually Work-Transformer九环节内部机制精读|How LLMs Actually Work-Transformer九环节内部机制精读]]》**：本文 4.2 节 Qwen3 thinking mode 拉低 BLEU 的根因——训练时 `ReasoningTemplate` 给 label 包 `<think>\n\n</think>\n\n{output}`、推理时 `enable_thinking=True` 把空 think 填满——本质是 Transformer generate 阶段对 think token 的处理差异。结合该文对 generate 环节内部机制的拆解，能更扎实地理解为什么关 `--enable_thinking=False` 能把 BLEU 从 1.82 拉回 46.32。

**《[[大厂技术文章-DailyTech/文章/GBrain-Agent时代知识自组织与自进化体系|GBrain-Agent时代知识自组织与自进化体系]]》**：本文"还没做好的几块"里提到要搞 `lessons.jsonl` 累计踩坑库、跨场景知识迁移（Query 改写经验迁移到同款判定），呼应 GBrain 主张的"团队共享调优知识资产"与知识自进化。两者都把"沉淀-复用-复利"作为 Agent 时代的核心方法论，可对照理解知识资产化的长期价值。

---

## 重难点理解

重点1: 三阶段流水线 — 场景诊断 → 方案设计 → 自动化实验，每阶段有明确产出物（analysis_report.md / improve_guide.md / experiment_results.csv+final_report.md），方便 Agent 在长上下文里"接力" — 误区：以为三阶段只是流程标签，实际每阶段的产出物是 Agent 下一阶段输入的硬性依赖，跳过会导致上下文断裂。

重点2: LOCKED 参数（MODEL_NAME 锁定）— 在阶段1体检后就锁定基座，整个实验序列里 `MODEL_NAME` 不再是可调变量，Agent 不允许擅自换基座，出现"LoRA 全跑过仍饱和"才起"需要决策"节提示用户拍板 — 误区：把基座当成可调变量让 Agent 自由探索，会导致实验矩阵爆炸且无法归因。

重点3: 6 级优先级调参清单 — 一级（LR/LoRA/epochs/Prompt/cutoff_len，直接影响效果）→ 二级（batch）→ 三级（训练策略）→ 四级（数据）→ 五级（分布式）→ 六级（评估），按优先级展开而非暴力网格搜索 — 误区：上来就调分布式/评估参数，这些是工程问题不是效果问题，应放在后面。

重点4: configs/*.sh 的三重角色 — 同一份脚本在本地是研究员改的入口、submodule 实验分支里是被 sync 过去的提交入口、远端容器里是真正被执行的训练启动脚本，靠 `sync_to_tuning_factory.sh` 自动同步 — 误区：在三处分别维护脚本导致不一致，应坚持"改一处就够"。

重点5: 平台问题外挂给平台 skill — 平台级报错（OOM/NCCL/镜像/队列额度/OSS endpoint/pod 调度）交给官方 `nebula-support` skill，业务级问题（DeepSpeed ckpt/Qwen3 thinking/oss_endpoint）留在项目 SKILL.md — 误区：把平台级 Debug 也写进项目 skill，导致重复维护且跟不上平台更新。

重点6: predict_with_generate 自动评估 — 加 `--do_predict --predict_with_generate=True --enable_thinking=False` 三行，Trainer 训完直接打 BLEU/ROUGE，省掉一整轮推理脚本 — 误区：仍手动起推理任务算指标，既慢又容易和训练 ckpt 对不上。

重点7: Qwen3 thinking mode 陷阱 — 训练时 `ReasoningTemplate` 给 label 包空 think，推理时 `enable_thinking` 默认 True 会把空 think 填满，导致 BLEU 拿思考内容跟 reference 算分惨跌到 1.82 — 误区：以为 eval_loss 低（0.004）就万事大吉，实际 generate 阶段行为和训练 loss 是两回事。

重点8: eff_batch 公式与调参顺序 — `eff_batch = world_size × per_device_train_batch_size × gradient_accumulation_steps`，先调 per_device 到显存极限再用 grad_accum 凑 eff_batch，最后 LR 按 √eff_batch 微调 — 误区：直接调 LR 而不动 batch 结构，导致收敛不稳。
...
```mermaid
flowchart TD
    U[用户输入: 研究目标+数据样例+预算] --> S1

    subgraph S1[阶段1 场景理解与基线分析]
        S1A[7维诊断: 基座/数据质量/Prompt/训练方式/Eval/Baseline复盘]
        S1B[跑baseline 拿eval_loss与任务指标]
        S1C[锁定MODEL_NAME LOCKED参数]
        S1D[产出 analysis_report.md]
        S1A --> S1B --> S1C --> S1D
    end

    S1 --> S2

    subgraph S2[阶段2 实验方案设计]
        S2A[从技术候选库15+技术点匹配候选]
        S2B[按6级优先级参数清单展开]
        S2C[产出 improve_guide.md]
        S2A --> S2B --> S2C
    end

    S2 --> S3

    subgraph S3[阶段3 自动化实验验证]
        S3A[单变量探索→正交组合→数据维度兜底]
        S3B[生成config]
        S3C[push实验分支 sync_to_tuning_factory.sh]
        S3D[submit.sh 提交 拿task_id]
        S3E[Agent /loop 监控星云HTTP API]
        S3F[训练完成 下载日志zip]
        S3G[正则提取 eval_loss/BLEU/ROUGE]
        S3H[写入 experiment_results.csv]
        S3I[Agent 汇总 final_report.md]
        S3A --> S3B --> S3C --> S3D --> S3E --> S3F --> S3G --> S3H --> S3I
    end

    S3 --> DEC{是否饱和或需换基座?}
    DEC -->|否| S2
    DEC -->|是| HU[起'需要决策'节 用户拍板]

    S3I --> END[闭环: 模型训练→线上服务→样本反馈]
```
...
1. 名称: LOCKED 参数思路 — 内容: 把"不该让 Agent 动"的东西（如基座 MODEL_NAME）显式标出来锁定，避免 Agent 越界探索 — 应用场景: 任何 Agent 驱动的实验框架，凡是不该被自动搜索的变量都要 LOCKED。

2. 名称: configs 一文件一方式 — 内容: 每种训练方式（SFT LoRA/全参/MoE/DPO/蒸馏/RL/推理）一个独立 .sh，脚本头部统一"输入区"变量，起新实验只改顶部十几行 — 应用场景: 多训练方式并存的 LLM 微调项目，需要快速复制起实验时。

3. 名称: submodule 分支实验隔离 — 内容: 每个实验对应 TuningFactory 一个分支，命名 `experiment-{编号}-{场景}-{方式}-{变量}`，`sync_to_tuning_factory.sh` 自动拉分支+复制 configs+push — 应用场景: 多实验并行/串行且互不干扰的分布式训练场景。

4. 名称: 平台 HTTP API 全链路监控 — 内容: 状态查询、实时 stdout/stderr、日志 zip 下载全走 curl，不打开浏览器；用 Agent `/loop 15m` 周期唤起检查 — 应用场景: 任何有元信息&日志接口的训练平台（星云/K8s/Slurm/PAI），把人从盯日志解放。

5. 名称: predict_with_generate 自动评估 — 内容: Trainer 自带 `--predict_with_generate=True`，训完直接打 BLEU/ROUGE，省一整轮推理脚本 — 应用场景: 用 LLaMA-Factory/TuningFactory 做 SFT/DPO 且需要生成式指标时。

6. 名称: 平台 skill 兜底方法论 — 内容: 平台级报错（OOM/NCCL/镜像/队列）外挂给官方 skill（如 `nebula-support`），业务级 Debug 留项目 SKILL.md — 应用场景: 任何有官方 skill 的训练平台，避免项目 skill 重复维护平台问题。

7. 名称: eff_batch 调参三步法 — 内容: 先 per_device_train_batch_size 调到显存极限 → 再 gradient_accumulation_steps 凑 eff_batch → LR 按 √eff_batch 微调 — 应用场景: 分布式 SFT/DPO/蒸馏调 batch 结构时。

8. 名称: Debug 表沉淀进 SKILL.md — 内容: 每个踩坑（PyTorch2.6+DeepSpeed 反序列化、Qwen3 thinking 拉低 BLEU、oss_endpoint 队列差异）补一行进 SKILL.md Debug 表，Agent 下次自动避坑 — 应用场景: 任何重复出现的训练故障，都应沉淀成 Agent 可读的 Debug 条目。
...
| 知识点 | 定义 | 关键要素 | 关联概念 |
| --- | --- | --- | --- |
| AutoResearch | 让 Agent 按 SKILL 文件驱动的实验清单，自主完成从假设到验证的循环 | SKILL.md / 实验清单 / Agent 自主循环 | Karpathy autoresearch / TAO / Pailitao |
| 三阶段流水线 | 场景诊断 → 方案设计 → 自动化实验，每阶段有明确产出物 | analysis_report.md / improve_guide.md / experiment_results.csv | LOCKED 参数 / 技术候选库 |
| LOCKED 参数 | 显式标出"不该让 Agent 动"的变量（如 MODEL_NAME） | 锁定时机 / 越界保护 / 决策提示 | 6 级调参清单 |
| 6 级调参清单 | LR/LoRA → batch → 训练策略 → 数据 → 分布式 → 评估的优先级分层 | 一级最敏感 / 优先级展开 / 非网格搜索 | eff_batch / lora_rank / lora_target |
| TuningFactory | LLaMA-Factory 的内部 Fork，封装 ODPS/内部存储/多队列 | submodule / openlm 分支 / configs 同步 | LLaMA-Factory / ROLL |
| DeepSpeed ZeRO-2/3 | 分布式训练显存切分策略，配合全参/LoRA 多卡 | ZeRO 阶段 / LossScaler / weights_only | PyTorch 2.6 / Megatron Turbo |
| Qwen3 thinking mode | Qwen3 默认开启的推理思考模式，generate 先吐 think 再给答案 | ReasoningTemplate / enable_thinking / BLEU 陷阱 | predict_with_generate / eval_loss |
| eff_batch | world_size × per_device_train_batch_size × gradient_accumulation_steps | 显存极限 / grad_accum 凑数 / LR 按 √微调 | SFT/DPO/蒸馏/GRPO 推荐区间 |
| 离线数据蒸馏 | 大模型（DashScope）生成 hard label → SFT 训小模型 | hard label / DashScope / 已验证 | 在线 KD / logit caching |
| 在线 KD | TuningFactory --stage distill，teacher 在线推理 student 学 | forward KL / teacher 显存 / 配置就绪未验证 | 离线蒸馏 / ROLL |
| GRPO | 基于可编程 reward 的 RL 训练方式，本项目接 ROLL 框架 | 可编程 reward / ROLL / 未验证 | DPO / SimPO |
| 平台 skill 兜底 | 平台级问题外挂官方 skill，业务级问题留项目 skill | nebula-support / a1 skill install / Debug 表分层 | SKILL.md / Agent 诊断 |
| /loop 循环验证 | Agent 自带定时唤起能力，周期检查 task 状态并自动收尾 | /loop 15m / 省 token / 自动写 csv | Loop Engineering / 星云 HTTP API |

---

## 可复用建议

1. 建议: 把训练流水线沉淀成 SKILL.md 交给 Agent — 适用场景: 任何重复性高的 LLM 微调/调参工作流 — 预期效果: 把写配置/提交/监控/抓日志/算指标让给 Agent，研究员专注数据清洗/Prompt 改写/换基座等判断性工作。

2. 建议: 用三阶段+明确产出物的结构组织 Agent 长上下文 — 适用场景: Agent 需要多轮接力的复杂研究任务 — 预期效果: 每阶段产出 md/csv 作为下阶段输入，避免长上下文里信息断裂。

3. 建议: 对不该让 Agent 动的变量显式 LOCKED — 适用场景: Agent 驱动的实验框架，基座/数据 schema/平台等不应被自动搜索 — 预期效果: 防止实验矩阵爆炸、归因清晰、关键决策交回用户。

4. 建议: configs 一文件一方式 + 头部统一输入区 — 适用场景: 多训练方式并存的微调项目 — 预期效果: 起新实验只改顶部十几行变量，复制即用，降低出错率。

5. 建议: 监控全走 HTTP API + Agent /loop 唤起 — 适用场景: 任何有元信息&日志接口的训练平台 — 预期效果: 不开浏览器、不手搓 while 循环，到点自动唤起收尾，省心省 token。

6. 建议: 用 predict_with_generate 把评估嵌进训练 — 适用场景: 用 LLaMA-Factory/TuningFactory 做生成式任务 — 预期效果: 训完直接拿 BLEU/ROUGE，省一整轮推理脚本。

7. 建议: 平台问题外挂官方 skill，业务问题留项目 skill — 适用场景: 任何有官方 skill 的训练平台 — 预期效果: 项目 skill 只维护业务专属 Debug，平台问题由官方 skill 跟进，维护成本下降。

8. 建议: 把踩坑沉淀成 Agent 可读的 Debug 表 — 适用场景: 重复出现的训练故障（反序列化/thinking mode/endpoint 差异） — 预期效果: Agent 下次自动避坑，团队踩坑复利化。

9. 建议: eff_batch 按"显存极限→grad_accum 凑→LR 按 √微调"三步调 — 适用场景: 分布式 SFT/DPO/蒸馏调 batch 结构 — 预期效果: 收敛更稳，避免直接调 LR 导致的不稳。

10. 建议: 跨场景踩坑库（lessons.jsonl）累计复用 — 适用场景: 多业务场景共用一个 Agent 框架 — 预期效果: Query 改写学到的经验自动迁移给同款判定，知识资产化复利。

---

## 实施办法

1. 体检与基线：先按 7 维诊断（基座/数据质量/数据规模/Prompt/训练方式/Eval/Baseline 复盘）过一遍任务，前六维在 baseline 前完成，跑 baseline 拿 eval_loss 与任务指标，锁定 MODEL_NAME 作为 LOCKED 参数。

2. 设计实验方案：从 15+ 技术候选库匹配候选，按 6 级优先级参数清单展开（一级 LR/LoRA/epochs/Prompt/cutoff_len 最敏感先做），产出 improve_guide.md 作为 Agent 实验依据。

3. 起新实验：复制一份最接近的 `configs/*.sh`（如 `sft_lora_multi.sh`），只改头部"输入区"十几行变量（MODEL_NAME/SCENARIO/EXP_NAME/ODPS_PROJECT/INPUT/EVAL_INPUT/LR 等）。

4. 同步与提交：跑 `scripts/sync_to_tuning_factory.sh` 自动在 submodule 拉实验分支、复制 configs、push 远端；再 `bash submit.sh experiment-{编号}-{场景}-{方式}-{变量}` 拿到 task_id。

5. 监控与收尾：用 Agent `/loop 15m 检查 task_id 状态` 周期唤起，通过星云 HTTP API（meta/log/download）拉状态与日志；训练完成后下载日志 zip 到 experiment_logs/。

6. 自动指标提取：从 worker stdout 用正则 `predict_(\w+)\s+=\s+([\d.]+)` 提取 eval_loss/predict_bleu-4/predict_rouge-l，追加写入 experiment_results.csv 一行。

7. 自动评估开关：configs 默认带 `--do_predict --predict_with_generate=True --enable_thinking=False`（Qwen3 必须关 thinking，否则 BLEU 惨跌），训完日志直接出 BLEU/ROUGE。

8. 研究级汇总：每轮研究跑完后由 Agent 汇总 final_report.md，对比各方案相对增益、给出下一轮建议或终止结论；csv 是实验级明细，final_report 是研究级总结。

9. 踩坑沉淀：每次失败任务的根因+修复补一行进 SKILL.md Debug 表（如 PyTorch2.6+DeepSpeed 用 `--overwrite_output_dir=True` 且禁用 `--load_best_model_at_end=True`；Qwen3 加 `--enable_thinking=False`；多队列 OSS endpoint 同步修改）。

10. 平台问题分流：装官方 skill（如 `a1 skill install nebula-support --agent claude --global`），平台级报错自动调起官方 skill 诊断；项目 SKILL.md 只维护业务专属 Debug 表。

11. 闭环迭代：若"LoRA 全跑过仍饱和"则起"需要决策"节提示用户拍板换基座；下一步把"模型训练"这一环和"样本反馈/线上服务"接上，完成请求→召回→...→训练→线上服务→请求的完整闭环。
