...
> [!check] 当前系统数据统计结果核对（以四月份数据为例）
> | 工厂 | 核对结果 | 差异说明 |
> | :---: | :---: | :--- |
> | 意大利、柏林工厂 | ✅ 一致 | 未套期保值头寸数据与线下手工计算一致 |
> | 法国工厂 | ⚠️ 差异 | 未套期保值头寸数据比线下数据多 2365.2KG（差异源自CRM单据数量增加） |
> | 门登工厂 | ❌ 较大出入 | 差异源自西班牙头寸数据未参与计算 |
...
>
> 1. 业务如何管理和核对升贴水缺失？
> 2. 是否需要倒算升贴水的方式（例如根据合同金额与LME价格反推）？
>
> *注：复杂升贴水场景（固定升贴水 + 百分比升贴水），正在设计开发中。*
...
>     - 半成品/成品：非主成分金额 = 入库日期 `Σ(Scorporo_金属 × Yield× 金属组成)`（Scorporo_Cu、Scorporo_Zn 等）
...
> 3. 为什么半成品/成品采用了Scorporo_金属计算非主成分金额？
>    > *AI解读：因为半成品/成品的非主成分金属（如黄铜中的铅、锌）已包含在配方成本中，若用LME会重复计价，故采用Scorporo_金属价格更合理。*
...
| 金属 | Total ADDER | Total Qty | Adder单价 |
| :---: | :---: | :---: | :---: |
| ZN | 3733.69 | - | 0.0437 |
| CU | -187.05 | 12530.47 | -0.0149 |
...
> 3. Delta 是商品的维度，Adder是元素的维度，Adder通过当月的收货拆分得到，两者关系如何定义更简便？
...
> [!info] 现有业务逻辑
> - **场景一**（发生点价，金额部分开票）：`Credit/Debit to be received = 点价金额 - 已开票金额`
> - **场景二**（未发生点价，金额部分开票）：`Credit/Debit to be received = 月底定价公式价格 - 已开票金额`（优先使用点价，否则用月结定价规则，而非暂估价格）
...
>
> > [!ai-note] AI解读 & 行业补充
> > 根据IAS 2存货，存货应按成本与可变现净值（NRV）孰低计量。NRV的估计需考虑：
> > - 完工成本（半成品适用）
> > - 销售费用
