...
- **《[[AI Agent系列｜深入解析Function Calling、MCP和Skills的本质差异与最佳实践|Function Calling、MCP和Skills的本质差异]]》**：本文的"工具 AI 化"部分与 MCP Server、Skill 的讨论直接呼应。作者提出的 CLI > MCP Server > Skill > GUI 优先级排序，是该文中 MCP 和 Skill 概念在 Java 微服务场景下的具体落地。

- **《[[Skills：从编程工具的配角到Agent研发的核心|Skills：从编程工具的配角到Agent研发的核心]]》**：该文强调 Skills 是 Agent 研发的核心，本文进一步指出 Skills 的效力取决于 Harness 环境——没有本地可运行的环境，Skills 调用的 CLI 工具和验证脚本都无法执行，Skills 就只是空中楼阁。

- **《[[如何构建和调优高可用性的Agent？浅谈阿里云服务领域Agent构建的方法论|阿里云服务领域Agent构建的方法论]]》**：该文讨论 Agent 的高可用性构建方法论，本文则聚焦于 Agent 开发阶段的高效迭代环境。两者结合可形成从开发到上线全链路的 Agent 工程质量保障。
