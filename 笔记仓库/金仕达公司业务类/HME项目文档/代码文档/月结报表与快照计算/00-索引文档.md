...
| # | 文档 | Controller | API路径 | 核心逻辑概述 |
|---|------|-----------|---------|-------------|
| 12 | [12-夜盘保值头寸统计表.md](12-夜盘保值头寸统计表-PositionMonitorSubReport.md) | `ReportController` | `/api/report/getPositionMonitorSubReport` | 从 business_lme_hedge 取数 → 加权平均价格 → 按金属分组汇总 |
| 13 | [13-金属头寸表.md](13-金属头寸表-MetalBollettinoReport.md) | `ReportController` | `/api/report/getMetalBollettinoReport` | 现货+期货+调整三源合并 → 采购/销售/头寸三大区域计算 |
| 14 | [14-库存价值报表.md](14-库存价值报表-DetailedEntriesValue.md) | `EOMStorageController` | `/api/eomstorage/listDetailedEntriesValue` | 4层CTE查询 → accounting+added value合并 → 发票调整 → 金属价值计算 |
| 15 | [15-收货明细表.md](15-收货明细表-ReceiptDetails.md) | `ReportController` | `/api/report/receiptDetails` | 入库登记明细 → 单位转换(KG/TO) → 升贴水关联 → 折扣计算 |
| 16 | [16-销售开票定价表.md](16-销售开票定价表-SalesEngagament.md) | `ReportController` | `/api/report/getSalesEngagament` | 销售订单行 → 已定价量-开票量-差值量 → 已定价未开票价值 |
| 17 | [17-采购库存定价表.md](17-采购库存定价表-ProcurementInventoryPricingTable.md) | `ReportController` | `/api/report/procurementInventoryPricingTable` | 采购订单行 → 3个CTE取数 → 点价/固定价/均价分场景计算 |
| 18 | [18-头寸汇总表.md](18-头寸汇总表-BollettinoSummaryReport.md) | `ReportController` | `/api/report/getBollettinoSummaryReport` | 现货+期货+调整三源分组 → 采购/销售/交易/LME数量汇总（吨） |
| 19 | [19-当月库存附加价.md](19-当月库存附加价-ListAddedValue.md) | `EOMStorageController` | `/api/eomstorage/listAddedValue` | eom_storage_added_value 直接查询 → 附加单价计算 → 冲销取反 |
...
| # | 文档 | 说明 |
|---|------|------|
| 11 | [11-数量价格金额计算公式汇总.md](11-数量价格金额计算公式汇总.md) | 所有报表/快照的数量、价格、金额字段计算公式汇总，方便快速查阅 |
...
flowchart TB
    ...
    subgraph SharedData["🗄️ 共享数据源"]
        ...
        D6["movement_quantity<br/>现货计价量"]
        D7["futures_movement_quantity<br/>期货计价量"]
        D8["document_items<br/>入库明细"]
    end
